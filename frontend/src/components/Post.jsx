...
const Post = ({ post }) => {
    ...
    const { mutate: createComment, isPending: isCreatingComment } = useMutation({
        ...
        onError: (err) => {
            toast.error(err.response.data.message || "Failed to create a comment")
        }
    })

    //  useMutation hook for likeing a post
    const { mutate: likePost, isPending: isLikedPost } = useMutation({
        mutationFn: async () => {
    ...
    return (
        <div className="bg-secondary rounded-lg shadow mb-4">
            <div className="p-4">
                ...
                { post.image && <img src={ post.image } alt="Post Image" className="rounded-lg w-full mb-4" /> }
